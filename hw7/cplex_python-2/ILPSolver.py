...
def generateObjectives(dfg,cost_table):
	...
	for sink in dfg.sinks:
		bstr=bstr+" - "+generateNodeStartTime(sink) + " - " + generateNodeLatency(sink) + " >= 0\n"
	return bstr
...
def generateNodeMappingConstraints(sorted_levels,cost_table):
	...
	#	for node in nodes:
	#		bstr = bstr + " + " +str(res.area) + " " + res.name + "_" +node.name
	#bstr = bstr + " = 640"
	return bstr
...
def generateResourceConcurrencyConstraints(dfg,cost_table):
	...
	for node in (cost_table.res_nodes.items()[1][1]):
			for step in range(1,max_latency):
				bstr = bstr + "\n" + node.name +"_" + str(step) + " - mpy1_" + node.name + "_" + str(step) +" - mpy2_" + node.name + "_" + str(step) + " = 0"
	return bstr

def generateResourceConcurrencyConstraint(res,nodes):
	bstr = '\n\\ \t Resource Concurrency Constraint for Resource {rid}\n'.format(rid=res.name)
	if res.name == "mpy2" or res.name == "mpy1":
		...
		for step in range(1,max_latency):
			...
			for node in nodes:
				...
				if res.name == "mpy2":
					end_step = end_step-1;
				if node == nodes[-1]:
					bstr = bstr + res.name + "_" + node.name + "_" + str(step)
				else:
					bstr = bstr + res.name + "_" + node.name + "_" + str(step) + " + "
			bstr = bstr  + " <= " + str(res.number) + "\n"
		for node in nodes:
			bvar = res.name + "_" + node.name
			bstr = bstr + "\n" + bvar
			binary_variables.append(bvar)
			for step in range(1,max_latency):
				bvar =  res.name + "_" +node.name + "_" + str(step)
				bstr = bstr + " - " + bvar 
				binary_variables.append(bvar)
			bstr = bstr + " = 0"
		return bstr 
